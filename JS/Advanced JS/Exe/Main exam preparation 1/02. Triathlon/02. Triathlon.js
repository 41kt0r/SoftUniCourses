class Triathlon {
    ...
    completeness(participant, condition) {
        ...
        this.listOfFinalists.push({
            name: participant,
            gender: this.participants[participant],
        });
        return `Congratulations, ${participant} finished the whole competition`;
    }

    rewarding(participant) {
        const hasFinished = this.listOfFinalists.some((finalist) => finalist.name === participant)
    ...
    showRecord(criteria) {
        ...
        if (criteria === 'all') {
            let resultmesage = `List of all ${this.name} finalists:\n`

            const sortedFinalists = this.listOfFinalists.sort((a, b) => a.name.localeCompare(b.name));

            sortedFinalists.forEach(
                finalist => resultmesage += `${finalist.name}\n`)

                return resultmesage.trim();
        }
